...
void FindClosestPointForEach(PointCloud& sourceCloud, cv::Mat& destPoints, std::vector<float>& distances, std::vector<size_t>& indices)
{
...
float GetStandardDeviation(std::vector<float>& data)
{
	float mean = 0.0f;
...
void RejectOutlierMatches(std::vector<Point3f>& matches1, std::vector<Point3f>& matches2, std::vector<float>& matchDistances, float maxStdDev)
{
	float distanceStandardDev = GetStandardDeviation(matchDistances);

	std::vector<Point3f> filteredMatches1;
	std::vector<Point3f> filteredMatches2;
...
float ICP(Point3f *verts1, int nVerts1, Point3f *verts2, int nVerts2, float *R, float *t, int maxIter)
{
	PointCloud cloud1;
	cloud1.pts = std::vector<Point3f>(verts1, verts1 + nVerts1);
	...
	{
		std::vector<Point3f> matched1, matched2;

		std::vector<float> distances(nVerts2);
		std::vector<size_t> indices(nVerts2);
		FindClosestPointForEach(cloud1, verts2Mat, distances, indices);

		std::vector<float> matchDistances;
		std::vector<int> matchIdxs(nVerts1, -1);
		for (int i = 0; i < nVerts2; i++)
		{
